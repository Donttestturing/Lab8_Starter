describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    for (let i = 1; i < prodItems.length; i++) {  
      ...
      const restOfData = await prodItems[i].getProperty('data');        //loop through the rest of of the product items
      // Convert that property to JSON
      const plainValueForData = await restOfData.jsonValue();
      // Make sure the title, price, and image are populated in the JSON
      if (plainValueForData.title.length == 0) { allArePopulated = false; }
      if (plainValueForData.price.length == 0) { allArePopulated = false; }
      if (plainValueForData.image.length == 0) { allArePopulated = false; }
      // Expect allArePopulated to still be true
      expect(allArePopulated).toBe(true);
    }
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const sRootOfProdItem = await prodItem.getProperty('shadowRoot')
    ...
    let textVal = await buttonText.jsonValue();
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      ...
      const elButton = await sRootOfProdItem.$('button'); 
      const buttonText = await elButton.getProperty('innerText');     
      const buttonTextVal = await buttonText.jsonValue();
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      ...
      const buttonText = await elButton.getProperty('innerText');     
      const buttonTextVal = await buttonText.jsonValue();
      expect(buttonTextVal).toBe("Add to Cart");
    }
